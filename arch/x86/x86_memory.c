...
void x86_set_page(uint32_t memory_size)
{
    ...
    uint32_t* pte_add;
    uint32_t* pde_add;
    uint32_t  kernel_page_add   = PAGE_TABLE_ADD;
    uint32_t  rl_ele_num_in_pte = memory_size / SIZE_OF_PAGE;
    uint32_t  rl_ele_num_in_pde = rl_ele_num_in_pte / ELE_NUM_IN_PDE;
    
    pde_add   = (uint32_t*)(kernel_page_add);
    pte_add   = (uint32_t*)(((uint32_t)pde_add)+SIZE_OF_PDE);

    temp = (uint32_t)pte_add;
    ...
    store_page((uint32_t*)kernel_page_add);
}
